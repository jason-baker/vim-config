...
" Last Change:	$Date: 2010/03/29 16:54:48 $
" Version:	$Revision: 1.7 $
" URL:		http://real.metasyntax.net:2357/cvs/cvsweb.cgi/Config/vim/syntax/scheme.vim
...
if exists("b:is_gambit") || exists("is_gambit")
    ...
    syn keyword schemeSyntax namespace
    syn keyword schemeFunc box box? unbox set-box!
    syn region GambitC matchgroup=schemeOther start=+(\@<=c-declare "+ end=+")\@=+ contains=@GambitC
    syn region GambitC matchgroup=schemeComment start=+c-declare\s*#<<\z(.*\)$+hs=s+9 end=+^\z1$+ contains=@GambitC
    syn region schemeSexpComment start='#;(' end=')' contains=schemeSexp,schemeComment
endif
